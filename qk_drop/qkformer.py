...
class LocalSpatialSimilarity(nn.Module):
    ...
    def forward(self, x, prev_drop_mask=None):  # x: [T,B,C,H,W]
        ...
        avg_kernel = torch.ones(C, C, 7, 7, device=x.device)/49.0
        local_mean = F.conv2d(x, avg_kernel, padding=3)             # [B,C,H,W]
...
class TokenSpikingTransformer(nn.Module):
    ...
    def forward(self, x):
        T,B,C,H,W = x.shape
        temporal_score = self.temporal_scorer(x)    # [T,B,H,W]
        spatial_score = self.spatial_scorer(x)      # [T,B,H,W]  
        ...
        slow_x = slow_x + self.tssa(slow_x)
...
class SpikingTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        final_score = (temporal_score+spatial_score).to(x.device)
...
class hierarchical_spiking_transformer(nn.Module):
    def __init__(self,
                 ...
                 k_values=[0.9, 0.9, 0.8, 0.8, 0.8, 0.8, 0.8, 0.75, 0.75, 0.75] #[0.8, 0.8, 0.8, 0.7, 0.7, 0.7, 0.7, 0.7, 0.7, 0.7]  #[0.95, 0.95, 0.85, 0.85, 0.85, 0.85, 0.8, 0.8, 0.8, 0.8],#[0.95,0.95,0.85,0.85,0.85,0.85,0.8,0.8,0.8,0.8] #[0.9,0.9,0.9,0.9,0.9,0.9,0.85,0.85,0.85,0.85], # [1., 1., 0.95, 0.95, 0.95, 0.95, 0.9, 0.9, 0.9, 0.9]     [0.95,0.95,0.85,0.85,0.85,0.85,0.8,0.8,0.8,0.8]
                 ):
        super().__init__()
        ...
        stage2 = nn.ModuleList([TokenSpikingTransformer(
            ...
            qk_scale=qk_scale, drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[j],
            norm_layer=norm_layer, sr_ratio=sr_ratios, k_value=(k_values[j+1]))
            for j in range(2)])
        ...
        stage3 = nn.ModuleList([SpikingTransformer(
            ...
            qk_scale=qk_scale, drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[j],
            norm_layer=norm_layer, sr_ratio=sr_ratios, k_value=(k_values[j+3]))
