...
import qkformer_metric as qkformer
...
def main(args):
    ...
    if args.eval:
        if hasattr(model_without_ddp, 'reset_ssim'):
            model_without_ddp.reset_ssim()
        test_stats = evaluate(data_loader_val, model, device)
        if hasattr(model_without_ddp, 'get_ssim'):
            ssim_res = model_without_ddp.get_ssim()
            print(f"Layer-wise SSIM: {json.dumps(ssim_res, indent=4)}")
