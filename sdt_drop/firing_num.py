...
def main():
    ...
    if not torch.cuda.is_available() and args.device.startswith("cuda"):
        _logger.warning("CUDA is unavailable, switching to CPU.")
        args.device = "cpu"
    args.world_size = 1
    ...
    model = create_model(
        ...
        T=args.time_steps,
        pretrained=True,
        # checkpoint_path="./output/train/20250308-180531-sdt_drop-data-cifar100-t-4-spike-lif/checkpoint-304.pth.tar",
        drop_rate=args.drop,
    ...
    # print(f'Computational complexity: {macs: .0f}')
...
def validate(
    ...
    if hasattr(real_model, 'reset_ssim'):
        real_model.reset_ssim()
    ...
    if hasattr(real_model, 'get_ssim'):
        ssim_res = real_model.get_ssim()
        if args.local_rank == 0:
            _logger.info(f"Layer-wise SSIM: {json.dumps(ssim_res, indent=4)}")
